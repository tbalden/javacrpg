...
/**
 * Window to select a skill/act form or object use in normal time. For casting spells/using objects
 * using object related skills etc.
 * Player can select a character and then select skill/actform/object/target character
 * @author pali
 *
 */
public class NormalActWindow extends PagedInputWindow {
	...
	public NormalActWindow (UIBase base) {
		super(base);
		...
		try {
	    	...
	    	// sword and shield sign // TODO use icon 
	...
	public void setPageData(int turnActType, PartyInstance party, EncounterInfo encountered, boolean playerInitiated)
	{
		...
		header.text = Language.v("normalActWindow.header."+langPostfix);
		header.activate();
		desc.text = Language.v("normalActWindow.desc."+langPostfix);
		desc.activate();
		this.party = party;
	...
	public void setupPage() {
		// TODO fill char select
		EncounterInfo inf = new EncounterInfo(base.core.gameState.player.theFragment,base.core.gameState.player.theFragment);
		TurnActUnitLineup lineUp = new TurnActUnitLineup(inf,3);
		ArrayList<EncounterUnit> encUnitList = new ArrayList<EncounterUnit>();
		for (PersistentMemberInstance ins:base.core.gameState.player.orderedParty)
		{
			ins.encounterData = new EncounterUnitData(base.core.gameState.player.theFragment,ins);
			ins.encounterData.turnActLineup = lineUp;
			encUnitList.add(base.core.gameState.player.theFragment);
		}
		inf.encountered.put(base.core.gameState.player.theFragment, new int[0][0]);
		inf.encounteredSubUnits.put(base.core.gameState.player.theFragment, encUnitList);
		inf.appendOwnSubUnits(base.core.gameState.player.theFragment, encUnitList);
		
		setPageData(Ecology.PHASE_TURNACT_COMBAT, base.core.gameState.player, inf, true);
