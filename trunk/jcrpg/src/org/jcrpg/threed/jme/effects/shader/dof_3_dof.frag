...
void main()
{
	...
	vec4 sum = vec4(0.0);
	float tapDepth = texture2D(depth, vTexCoord).r;
	
	if (tapDepth<0.5)
	{
		sum = texture2D(scene, vTexCoord);
		gl_FragColor = sum;
	} else
	{
		...
		for (int i = 0; i < NUM_TAPS; ++i)
		{
			...
			vec2 coordScene = vTexCoord.st + (pixelSizeScene * poisson[i] * discRadiusScene);
			vec2 coordBlur = vTexCoord.st + (pixelSizeBlur * poisson[i] * discRadiusBlur);
